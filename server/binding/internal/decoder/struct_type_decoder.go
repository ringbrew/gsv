...
func (d *structTypeFieldTextDecoder) Decode(input *DecodeInput) error {
	var err error
	var text string
	var exist bool
	...
	if field.Kind() == reflect.Ptr {
		...
		if err != nil {
			logger.Info(logger.NewEntry().WithMessageF("unable to decode '%s' as %s: %v, but it may not affect correctness, so skip it", text, d.fieldType.Name(), err))
			return nil
		}
