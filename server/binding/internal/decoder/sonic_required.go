...
func checkRequireJSON(decodeInput DecodeInput, tagInfo TagInfo) bool {
	...
	node, _ := sonic.Get(decodeInput.Data, stringSliceForInterface(tagInfo.JSONName)...)
	if !node.Exists() {
...
func keyExist(decodeInput DecodeInput, tagInfo TagInfo) bool {
	if utils.FilterContentType(decodeInput.ContentType.ContentType()) != consts.MIMEApplicationJSON {
		return false
	}
